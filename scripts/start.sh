...
echo "> build 파일 복사" >> /home/lolmatch/logs/deploy.log
DEPLOY_PATH=/apps/lolmatch/
cp $BUILD_JAR $DEPLOY_PATH
echo "> build 파일 복사 완료 $DEPLOY_PATH" >> /home/lolmatch/logs/deploy.log
echo "> 현재 실행중인 애플리케이션 pid 확인" >> /home/lolmatch/logs/deploy.log
CURRENT_PID=$(pgrep -f $JAR_NAME)

if [ -z $CURRENT_PID ]
then
  echo "> 현재 구동중인 애플리케이션이 없으므로 종료하지 않습니다." >> /home/lolmatch/logs/deploy.log
else
